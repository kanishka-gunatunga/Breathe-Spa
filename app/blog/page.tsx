    ...
    return (
        <div>
            ...
            <div>
                <Image
                    className={`m-0 p-0 ${styles.blogCon}`}
                    src={urlFor(blogs.heroImage).url() || "/blog_banner.png"}
                    width={0}
                    height={0}
                    sizes="100%"
                    style={{width: '100%', height: 'auto'}}
            ...
            <div className={`${styles.contactContainer} section py-5 px-4 px-lg-5`}>
                ...
                <div className="row g-4" id="blogArticles">
                    {posts.map((post:BlogData,index: number) => (
                        <div key={index} className="col-md-4 mobile_padding_remove">
                            {/* <BlogCard post={post}/> */}
                            <BlogCard
                                title={post.title}
                                slug={typeof post.slug === "string" ? post.slug : post.slug.current}
                                featuredText={post.featuredText}
                                mainImage={post.featureImage}
                                body={post.body}
                                publishedAt={post.publishedAt} _id={post._id}/>
