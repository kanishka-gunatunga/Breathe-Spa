...
import Image from "next/image";
import styles from "@/styles/page.module.css";
import {getBlogPost} from "@/sanity/libs/api";
import {urlFor} from "@/sanity/libs/sanity";
import {PortableText} from "next-sanity";
import style from "@/styles/services.module.css";
import Link from "next/link";
...
export async function generateMetadata({params}: { params: Promise<{ slug: string }> }): Promise<Metadata> {
    ...
    return {
        title: mdata?.title || `Breathe Spa - ${blog.title}`,
        description: mdata?.description || blog.featuredText || 'Read the latest wellness insights from Breathe Spa.',
        keywords: mdata?.keywords?.join(', ') || 'spa, wellness, blog',
        openGraph: {
            title: mdata?.ogTitle || mdata?.title || `Breathe Spa - ${blog.title}`,
            description: mdata?.ogDescription || mdata?.description || blog.featuredText || 'Discover wellness insights from Breathe Spa.',
            images: mdata?.ogImage ? urlFor(mdata.ogImage).url() : blog.mainImage ? urlFor(blog.mainImage).url() : '/Rectangle4422.png',
            url: `https://breathespa.vercel.app/blog/${slug}`,
            type: 'article',
        },
    };
}
